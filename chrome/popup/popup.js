...
document.getElementById('rupee-btn').addEventListener('click', () => {
    document.getElementById('rupee-btn').classList.toggle('active');
    chrome.tabs.query({ active: true, lastFocusedWindow: true }, tabs => {
        let url = tabs[0].url;
        let components = url.split(/[\/?]/); // Use regex to split with "/" or "?"
        let productSN = components[5]; // Use the 5th element
        if (productSN.includes("ref")) {
            productSN = components[4]; // Use the 4th element if the 5th contains "ref"
        }
